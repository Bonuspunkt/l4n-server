...
class LobbyRepo extends EventEmitter {
        ...
        this.statements = {
            ...
            spaceInLobby: db.prepare(`
                 SELECT *
                  ...
                  WHERE id = $lobbyId
                    AND maxPlayers > (
                         SELECT count(*) FROM LobbyUser
                          WHERE lobbyId = $lobbyId AND left is null
                        )`),
            ...
            updateState: db.prepare(`
                 UPDATE Lobby
                  ...
                  WHERE id = $lobbyId
            `),
            updatePrivateInfo: db.prepare(`
                 UPDATE Lobby
                    SET privateInfo = $privateInfo
                  WHERE id = $lobbyId
            `),
        };
    }
    ...
    leave({ lobbyId, userId }) {
        const { byUserId, leaveLobby, allLobbyUsers } = this.statements;
    ...
    changeState({ lobbyId, newState, userId, privateInfo }) {
        const { byId, updateState, updatePrivateInfo } = this.statements;
        const lobby = byId.get({ lobbyId });
        ...
        if (
            ...
            let patch = { id: lobbyId, state: newState };

            if (privateInfo) {
                updatePrivateInfo.run({ lobbyId, privateInfo });
                patch = { ...patch, privateInfo };
            }
