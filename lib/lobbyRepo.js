...
class LobbyRepo extends EventEmitter {
    constructor(resolve) {
        ...
        this.statements = {
            insertLobby: db.prepare(`
                INSERT INTO Lobby (name, provider, userId, game, minPlayers, maxPlayers, publicInfo, privateInfo)
                VALUES ($name, $provider, $userId, $game,  $minPlayers, $maxPlayers, $publicInfo, $privateInfo);`),
            ...
            checkSpawnCondition: db.prepare(`
                 SELECT *
                   FROM Lobby
                  WHERE 1 = 1
                    AND Id = $lobbyId
            ...
            checkResetCondition: db.prepare(`
                 SELECT *
                   FROM Lobby
                  WHERE 1 = 1
                    AND Id = $lobbyId
    ...
    join({ lobbyId, userId }) {
        const { insertLobbyUser, checkSpawnCondition, updateState } = this.statements;
        ...
        const shouldSpawn = checkSpawnCondition.get({ lobbyId });

        if (shouldSpawn) {
            updateState.run({ lobbyId, state: STATE.READY });

            this.emit('state', { lobbyId, state: STATE.READY });
        }
    }

    leave({ lobbyId, userId }) {
        const { byUserId, leaveLobby, checkResetCondition, updateState } = this.statements;
        ...
        const shouldResetState = checkResetCondition.get({ lobbyId });
        if (shouldResetState) {
            updateState.run({ lobbyId, state: STATE.WAITING });

            this.emit('state', { lobbyId, state: STATE.WAITING });
        }
    }
