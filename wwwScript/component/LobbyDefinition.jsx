...
const FormInput = props => {
    switch (props.type) {
        ...
        case 'select':
            const options = props.values.map(value => <option key={ value.name }>{ value.name }</option>);
            return (
...
class LobbyDefinition extends PureComponent {
    render() {
        ...
        const formItems = Object.keys(lobbyDefinition)
            .map(key => <FormItem key={ key } name={ key } { ...lobbyDefinition[key] } />);
